...
class GenderViewTestCase(APITestCase):
    ...
    def test_get_position_by_builder_user(self) -> None:
        """
        `builder`グループのユーザーが「役職」を取得できるかテスト
        """
        # `builder`グループユーザーで認証
        self.client.force_authenticate(user=self.builder_user)

        url = reverse("position")  # `urls.py`で定義された名前を使用
    ...
    def test_get_position_by_normal_user(self) -> None:
        """
