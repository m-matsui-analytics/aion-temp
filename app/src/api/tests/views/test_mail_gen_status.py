...
class MailGenStatusViewTestCase(APITestCase):
    """
    Test case for `MailGenStatusView`.
    """
    ...
    def test_get_mail_gen_status_by_builder_user(self) -> None:
        """
        `builder`グループのユーザーが「メール生成ステータス」を取得できるかテスト
        """
        # `builder`グループユーザーで認証
        self.client.force_authenticate(user=self.builder_user)

        url = reverse("mail_gen_status")  # `urls.py`で定義された名前を使用
    ...
    def test_get_mail_gen_status_by_normal_user(self) -> None:
        """
