...
class MailGenErrorTypeViewTestCase(APITestCase):
    """
    Test case for `MailGenErrorTypeView`.
    """
    ...
    def test_get_mail_gen_error_type_by_builder_user(self) -> None:
        """
        `builder`グループのユーザーが「メール生成エラータイプ」を取得できるかテスト
        """
        # `builder`グループユーザーで認証
        self.client.force_authenticate(user=self.builder_user)

        url = reverse("mail_gen_error_type")  # `urls.py`で定義された名前を使用
        ...
        # 期待される値を直接定義
        expected_choices = [
            {"value": 1, "label": "情報不足エラー"},
            {"value": 2, "label": "マッチングエラー"},
            {"value": 3, "label": "システムエラー"},
        ]
    ...
    def test_get_mail_gen_error_type_by_normal_user(self) -> None:
        """
        ...
        url = reverse("mail_gen_error_type")
        response = self.client.get(url)
